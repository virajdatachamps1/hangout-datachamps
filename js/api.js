...
const API_SECRET = 'datachamps_hangout_2025'; // Simple API secret - you can change this
...
class APIManager {
  ...
  async makeRequest(action, data = {}, method = 'POST') {
    try {
      // Get user email instead of JWT token
      ...
      const requestData = {
        ...
        data,
        user: userEmail,
        apiSecret: this.apiSecret, // Send API secret instead of JWT
      ...
      const options = {
        ...
        headers: {
          ...
          'X-API-Secret': this.apiSecret // Also send in header
        }
      };
      ...
      console.log('Making API request:', action, 'for user:', userEmail);
  ...
  // Training Management
  async getTraining() {
    return await this.makeRequest('getTraining', {}, 'GET');
  }

  async getUpcomingTraining(limit = 10) {
    return await this.makeRequest('getUpcomingTraining', { limit }, 'GET');
  }
